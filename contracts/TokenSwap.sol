...
contract TokenSwap is AccessControl {
    ...
    function swapCbToCbs(bytes32 partition, uint256 value, bytes calldata data) public onlyRole(SWAP_CB_TO_CBS_ROLE) returns (uint256) {
        require(value > 0, string.concat("Amount of ", cbToken.symbol(), " must be greater than zero"));
        require(cbToken.balanceOf(msg.sender) >= value, string.concat("Sender does not have enough ", cbToken.symbol()));
        require(
    ...
    function swapCbsToCb(bytes32 partition, uint256 value, bytes calldata operatorData) public onlyRole(SWAP_CBS_TO_CB_ROLE) returns (uint256) {
